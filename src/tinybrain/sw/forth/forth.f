...
: ELSE IMMEDIATE
    ' BRANCH , \ branch forward until the alternative is done.
    HERE @     \ We don't yet know where we need to jump to so, we need to fix it up later
    0 ,	\ dummy offset for now
    SWAP       \ Fixup the branch from the 0BRANCH target given by IF
    DUP HERE @ SWAP - SWAP !
;
